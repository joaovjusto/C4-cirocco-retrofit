...
//   Serial.println("protothreadMainLoop");
//   PT_BEGIN(pt);
//   while (1) {
...
//       int isButtonCLicked = checkButtonClick();
...
static int protothreadAmbianceKeeper(struct pt *pt) {
  PT_BEGIN(pt);
  Serial.println("PROTOTHREAD 2");
  while (1) {
    //    Serial.println("antes do esperar");
    //    Serial.println(isButtonPushed);
    //    PT_WAIT_UNTIL(pt, isButtonPushed == 1);
    //    Serial.println("depois do esperar");
    if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK)  // If there are no errors, continue
    {
      // int isButtonCLicked = checkButtonClick();
...
//         sendEspStateEcu();
//       } else {
//         Serial.println("BOTAO CLICADO RAPIDO");
...
inline void sendAmbianceToCluster() {
  ...
  canAmbiance.data[0] = theme & 0x03;                    //theme value is only in bit0&1 =0x03 mask  0xFC is reseting SndData -->  DDDDDD00 | 000000TT   D=original data, T=theme
  // canAmbiance.data[1] = ((canAmbiance.data[1] & 0x3F) | (ambiance & 0xC0));                  //ambiance value is only in bit 6&7=0xC0 mask
  canAmbiance.data[1] = ambiance;                 //ambiance value is only in bit 6&7=0xC0 mask
